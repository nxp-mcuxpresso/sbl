...
set demo_region1_arg=region1_arg=1,[0x70101000,0x3000,0]
...
if %signing_type% == HAB (
    ...
    image_enc hw_eng=bee ifile=.\ivt_sfw2.bin ofile=.\sfw_2_enc.bin base_addr=0x70100000 ^
                    %region_key% %demo_region1_arg% use_zero_key=1 is_boot_image=0

    :: Add mcuboot header
    python %imgtool_path%\imgtool.py create --align 4  --version "1.1"  --header-size %mcu_header_size% --pad-header --slot-size 0x100000 --key-info .\ehdr1.bin .\ivt_sfw2_nopadding.bin .\sfw_2_sign.bin
    ...
    image_enc.exe hw_eng=bee ifile=.\sfw_2.bin  ofile=.\sfw_2_enc.bin  base_addr=0x70100000 ^
                  %region_key% %demo_region1_arg% use_zero_key=1 is_boot_image=0

    :: Add mcuboot header and tlv
    if %signing_type%==RSA2048 (
        python %imgtool_path%\imgtool.py sign --key %imgtool_path%\sign-rsa2048-priv.pem --align 4 --version "1.1" --header-size  %mcu_header_size% --pad-header --slot-size 0x100000 --max-sectors 32 --key-info .\ehdr1.bin  .\sfw2.bin  .\sfw_2_rsa.bin
    ) else (
        python %imgtool_path%\imgtool.py sign --key %imgtool_path%\sign-ecdsap256-priv.pem --align 4 --version "1.1" --header-size  %mcu_header_size% --pad-header --slot-size 0x100000 --max-sectors 32 --key-info .\ehdr1.bin  .\sfw2.bin  .\sfw_2_rsa.bin
    )
    
    :: Merge mcuboot header and tlvs into encrypted image 
    python img_helper.py merge --header-size %mcu_header_size% --sign-image .\sfw_2_rsa.bin --enc-image .\sfw_2_enc.bin
