...
if %signing_type% == HAB (
    ...
    if %signing_type%==RSA2048 (
        python %imgtool_path%\imgtool.py sign --key %imgtool_path%\sign-rsa2048-priv.pem --align 4 --version "1.1" --header-size %mcu_header_size% --slot-size 0x100000 --max-sectors 32 --key-info .\ehdr1.bin  .\sfw_2_enc.bin  .\sfw_2_rsa.bin
    ) else (
        python %imgtool_path%\imgtool.py sign --key %imgtool_path%\sign-ecdsap256-priv.pem --align 4 --version "1.1" --header-size %mcu_header_size% --slot-size 0x100000 --max-sectors 32 --key-info .\ehdr1.bin  .\sfw_2_enc.bin  .\sfw_2_rsa.bin
    )
