...
enum _checkstatus_id
{
    kCheckStatus_Crc = 0,
    kCheckStatus_LastError = 1,
};
...
#ifndef BL_FEATURE_PROPERTY_RESERVED_REGION_COUNT
#define BL_FEATURE_PROPERTY_RESERVED_REGION_COUNT (2)
#endif

#ifndef BL_FEATURE_PROPERTY_RAM_REGION_COUNT
...
{
    kProperty_ReservedRegionsCount = BL_FEATURE_PROPERTY_RESERVED_REGION_COUNT,
...
typedef struct BootloaderConfigurationData
{
    ...
    uint32_t enabledPeripherals_extern; //!< [34:27] enabledPeripherals_final = enabledPeripherals_extern << 8 |
                                        //! enabledPeripherals;
...
enum _external_memory_property_tag
{
    ...
    kExternalMemoryPropertyTag_MemorySizeInBytes = 0x10, //!< Total memory size tag in terms of byte
...
typedef struct PropertyStore
{
    ...
    irq_notifier_pin_property_store_t irqNotifierPin; //!< GPIO port&pin used as IRQ notifier pin.
#endif
    uint32_t ffrKeystoreUpdateOpt;
} property_store_t;
...
//! @brief Update SoC Specified properties as needed.
