...
const volatile uint32_t s_defaultByteWriteTimeoutMs = kDefaultByteWriteTimeoutMs;
#else
#define s_defaultByteWriteTimeoutMs (kDefaultByteWriteTimeoutMs)
#endif
...
static status_t update_ping_response_crc16(ping_response_framing_packet_t *pingFramingResponse);
                            ...
                            packet_type_t packetType)
{
    ...
    *packetLength = 0;
    ...
    do
    {
        ...
        memset_s(g_serialContext.data, sizeof(g_serialContext.data), 0, sizeof(g_serialContext.data));
...
status_t serial_send_ping_response(const peripheral_descriptor_t *peripheral)
{
    ...
    {
        ping_response_framing_packet_t response;
        response.header.startByte = kFramingPacketStartByte;
        response.header.packetType = kFramingPacketType_PingResponse;
        memcpy(&response.data, &k_PingResponse, sizeof(ping_response_t));
        update_ping_response_crc16(&response);
        peripheral->byteInterface->write(peripheral, (uint8_t *)&response, sizeof(ping_response_framing_packet_t),
                                         sizeof(ping_response_framing_packet_t) * s_defaultByteWriteTimeoutMs);
    }
...
static status_t write_data(const uint8_t *buffer, uint32_t byteCount)
{
    ...
    retVal = g_bootloaderContext.activePeripheral->byteInterface->write(
        g_bootloaderContext.activePeripheral, buffer, byteCount, byteCount * s_defaultByteWriteTimeoutMs);
...
static status_t read_data(uint8_t *buffer, uint32_t byteCount, uint32_t timeoutMs)
{
    if (g_bootloaderContext.activePeripheral->byteInterface->read != NULL)
    {
        // If peripheral offers a read function, then read the data directly.
...
static status_t read_data_packet(framing_data_packet_t *packet, uint8_t *data, packet_type_t packetType)
{
    ...
    if (packet->length > 0)
    {
        ...
        memset_s(data, packet->length, 0, packet->length);
...
static status_t read_start_byte(framing_header_t *header)
{
    ...
    do
    {
        status = read_data(&header->startByte, 1, 0); // no timeout for first byte of packet
        if (status != kStatus_Success)
        {
            break;
        }
...
//! @brief Calculate crc16 for ping response
static status_t update_ping_response_crc16(ping_response_framing_packet_t *pingFramingResponse)
{
    if (pingFramingResponse == NULL)
    {
        return kStatus_InvalidArgument;
    }
    ...
    crc16_init(&crcInfo);
    // Run CRC on all header bytes besides the CRC field
    crc16_update(&crcInfo, (uint8_t *)pingFramingResponse, sizeof(ping_response_framing_packet_t) - sizeof(uint16_t));
    // Finalize the CRC calculations
    crc16_finalize(&crcInfo, &crc16);

    pingResponse = (ping_response_t *)&pingFramingResponse->data[0];
    pingResponse->crc16 = crc16;
