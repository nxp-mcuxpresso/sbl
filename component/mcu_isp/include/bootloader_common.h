...
#define PTR_ADDR(ptr) ((uint32_t)(ptr))
...
#if defined(BL_TARGET_RTL) || defined(BL_TARGET_ZEBU) || defined(BL_TARGET_FPGA)
extern void debug_printf(const char *format, ...);

#else // #if BL_TARGET_RTL
...
#endif // #if BL_TARGET_RTL
#else  // #if (defined(DEBUG) || defined(_DEBUG) || defined(FORCE_DEBUG)) && !defined(DEBUG_PRINT_DISABLE)
...
#define kStatusGroup_ApplicationRangeStart_BL (100)
...
{
    kStatusGroup_Bootloader = kStatusGroup_ApplicationRangeStart_BL, //!< Bootloader status group number (100).
...
{
    kHabStatus_Fab = 0x48414200,
    kHabStatus_Open = 0x484142f0,
    kHabStatus_FA = 0x48414233,
    kHabStatus_Close = 0x404142cc,
} habstatus_option_t;

//! @brief Bootloader core type
typedef enum _bootloader_core_type
{
    kCoreType_Core0 = 0,
    kCoreType_Core1 = 1,
    kCoreType_All = 0xFFu,
} bootloader_core_type_t;

//! @brief Bootloader memory property
typedef enum _bootloader_memory_property
{
    kMemoryProperty_NonVolatile = 0,
    kMemoryProperty_Volatile = 1,
    kMemoryProperty_Device = 2,
    kMemoryProperty_All = 0xFFu,
} bootloader_memory_property_t;

//! @brief Bootloader expand memory map property
typedef struct _expand_memory_map_property
{
    bootloader_core_type_t coreType;
    bootloader_memory_property_t memoryProperty;
} expand_memory_map_property_t;
...
//! @brief Update properties based on specific chips
...
//!@brief Secure memset implementation
int memset_s(void *, size_t, int, size_t);

//!@brief ISP Cleanup utilities
