...
static usb_device_composite_struct_t *s_device_composite;
    ...
    switch (event)
    {
        case kUSB_DeviceMscEventReadResponse:
            ...
            if (lba_data_ptr != NULL)
            {
                ...
                {
                    s_device_composite->msc_disk.msc_state.read_write_error = 1;
                }
            }
        ...
        case kUSB_DeviceMscEventWriteRequest:
            ...
            // RAM disk
            //        lba_data_ptr->buff_ptr = s_device_composite->msc_disk.storage_disk + lba_data_ptr->offset;
            if (lba_data_ptr != NULL)
            {
                lba_data_ptr->buffer = (uint8_t *)s_device_composite->msc_disk.msc_state.buffer;
            }
        ...
        case kUSB_DeviceMscEventRemovalRequest:
            ...
            {
                s_device_composite->msc_disk.msc_state.isDiskLocked = *prevent_removal_ptr;
            }
...
{
    //   s_device_composite->msc_disk.current_configuration = configure;
...
{
    s_device_composite = device_composite;
    //    s_device_composite->msc_disk.storage_disk[0] = 0x01;   // RAM disk

    // Get the length of the info.txt file by writing it to a buffer temporarily.
    usb_msd_update_info_file(s_device_composite->msc_disk.msc_state.buffer);
    uint32_t length = strlen((char *)s_device_composite->msc_disk.msc_state.buffer);

    // Init the state info. This also clears the sector buffers.
    memset(&s_device_composite->msc_disk.msc_state, 0, sizeof(s_device_composite->msc_disk.msc_state));

    s_device_composite->msc_disk.msc_state.infoFileLength = length;

    s_device_composite->msc_disk.msc_state.start_app = false;
    return kStatus_USB_Success;
}
...
usb_status_t usb_device_msc_disk_deinit(usb_device_composite_struct_t *device_composite)
{
    s_device_composite = device_composite;
    // RAM Disk
    // s_device_composite->msc_disk.storage_disk[0] = 0x01;
    s_device_composite->msc_disk.msc_handle = (class_handle_t)NULL;
    return kStatus_USB_Success;
}
...
void usb_device_msc_disk_pump(void)
{
    ...
    if (s_device_composite->msc_disk.msc_state.sbLoaderStatus == kSbLoaderFree)
    {
        sbloader_init();
        s_device_composite->msc_disk.msc_state.sbLoaderStatus = kSbLoaderInited;
        s_device_composite->msc_disk.msc_state.start_app = true;
    }

    if (s_device_composite->msc_disk.msc_state.isProcessing)
    {
        ...
        do
        {
            s_device_composite->msc_disk.msc_state.sbLoaderStatus = kSbLoaderLoading;
            status = feed_sbloader();
        } while (status == kStatus_Success);
        if ((s_device_composite->msc_disk.msc_state.remainingFileLength == 0) &&
            (s_device_composite->msc_disk.msc_state.sbLoaderStatus == kSbLoaderLoading))
        {
        ...
        {
            s_device_composite->msc_disk.msc_state.isProcessing = true;
        }
        ...
        {
            s_device_composite->msc_disk.msc_state.isProcessing = false;

            if (status == kStatusRomLdrSectionOverrun)
            {
                s_device_composite->msc_disk.msc_state.transferStatus = kSuccessStatus;
            ...
            {
                s_device_composite->msc_disk.msc_state.transferStatus = kFailureStatus;
...
usb_status_t usb_msd_write_sector(uint32_t sector, const usb_device_lba_app_struct_t *lbaData)
{
    if (sector >= kFirstUnusedSector)
    {
        ...
        if (!s_device_composite->msc_disk.msc_state.isTransferring)
        {
            if (s_device_composite->msc_disk.msc_state.isProcessing)
            {
        ...
        else if (sector == s_device_composite->msc_disk.msc_state.nextSector)
        {
            saveSbData = true;
        }
        else
        ...
        if (saveSbData)
        {
            ...
            uint32_t count = MIN(s_device_composite->msc_disk.msc_state.remainingFileLength, lbaData->size);
            s_device_composite->msc_disk.msc_state.dataBytesAvailable += count;
            s_device_composite->msc_disk.msc_state.remainingFileLength -= count;

            s_device_composite->msc_disk.msc_state.nextSector = sector + 1;

            if (s_device_composite->msc_disk.msc_state.remainingFileLength == 0)
            {
                s_device_composite->msc_disk.msc_state.isTransferring = false;
            }

            memcpy(s_device_composite->msc_disk.msc_state.sbBuffer, lbaData->buffer, count);
...
usb_status_t usb_msd_read_sector(uint32_t sector, usb_device_lba_app_struct_t *lbaData)
{
    ...
    switch (sector)
    {
        ...
        case kRootDir1Sector:
        {
            ...
            rootDir[kInfoFileDirEntry].entry.fileSize = s_device_composite->msc_disk.msc_state.infoFileLength;

            const char *statusMessage = kStatusMessages[s_device_composite->msc_disk.msc_state.transferStatus];
...
bool is_valid_sb_file_header(const uint8_t *data, uint32_t *fileLength)
{
    ...
    if ((header->m_signature == BOOT_SIGNATURE) && (header->m_signature2 == BOOT_SIGNATURE2) &&
        ((header->m_majorVersion == SB_FILE_MAJOR_VERSION) || (header->m_majorVersion == SB_FILE_MAJOR_VERSION2))
    {
        assert(fileLength);
...
status_t feed_sbloader(void)
{
    ...
    uint32_t available = MIN(s_device_composite->msc_disk.msc_state.dataBytesAvailable, kDiskSectorSize);
    ...
    {
        s_device_composite->msc_disk.msc_state.dataBytesAvailable -= available;

        // Give the data to sbloader.
        status = sbloader_pump(s_device_composite->msc_disk.msc_state.sbBuffer, available);
    }
...
void usb_msd_start_transfer(uint32_t length)
{
    s_device_composite->msc_disk.msc_state.isActive = true;
    s_device_composite->msc_disk.msc_state.isTransferring = true;
    s_device_composite->msc_disk.msc_state.sbLoaderStatus = kSbLoaderFree;
    s_device_composite->msc_disk.msc_state.isProcessing = true;
    s_device_composite->msc_disk.msc_state.isReceivePending = false;
    s_device_composite->msc_disk.msc_state.dataBytesAvailable = 0;
    s_device_composite->msc_disk.msc_state.remainingFileLength = length;
    s_device_composite->msc_disk.msc_state.transferStatus = kTransferringStatus;
}
...
void reset_usb_msd_state(uint8_t loaderStatus)
{
    s_device_composite->msc_disk.msc_state.isActive = true;
    s_device_composite->msc_disk.msc_state.isTransferring = false;
    s_device_composite->msc_disk.msc_state.sbLoaderStatus = loaderStatus;
    s_device_composite->msc_disk.msc_state.isProcessing = false;
    s_device_composite->msc_disk.msc_state.isReceivePending = false;
    s_device_composite->msc_disk.msc_state.dataBytesAvailable = 0;
    s_device_composite->msc_disk.msc_state.remainingFileLength = 0;
    s_device_composite->msc_disk.msc_state.transferStatus = kTransferringStatus;
}
