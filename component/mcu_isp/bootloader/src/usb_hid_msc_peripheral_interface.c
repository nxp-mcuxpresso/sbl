...
bool usb_hid_poll_for_activity(const peripheral_descriptor_t *self)
{
    ...
    debug_printf("Bootloader: usb_hid_poll_for_activity() \n");
...
usb_status_t usb_device_callback(usb_device_handle handle, uint32_t event, void *param)
{
    ...
    debug_printf("Bootloader: usb_device_callback, event = %x \n", event);
    switch (event)
    {
...
status_t usb_device_full_init(const peripheral_descriptor_t *self, serial_byte_receive_func_t function)
{
    ...
    debug_printf("Bootloader: usb_device_full_init()\n");
    ...
    if (!usb_clock_init(0))
    {
        ...
        debug_printf("Bootloader: usb_device_full_init() ---- usb_clock_init Failed\n");
        return kStatus_Fail;
    }

    property_store_t *propertyStore = g_bootloaderContext.propertyInterface->store;
    debug_printf("Bootloader: usb_device_full_init() ---- g_bootloaderContext.propertyInterface->store \n");
    ...
    debug_printf("Bootloader: usb_device_full_init() ---- usbStringsPointer \n");
    ...
    debug_printf("Bootloader: usb_device_full_init() ---- g_device_composite.speed = USB_SPEED_FULL \n");
    g_device_composite.speed = USB_SPEED_FULL;
    g_device_composite.attach = 0;
#if USB_DEVICE_CONFIG_HID
     debug_printf("Bootloader: usb_device_full_init() ---- USB_DEVICE_CONFIG_HID \n");
    ...
    debug_printf("Bootloader: usb_device_full_init() ---- g_device_composite.device_handle = NULL \n");
    if (kStatus_USB_Success !=
        USB_DeviceClassInit(CONTROLLER_ID, &g_composite_device_config_list[0], &g_device_composite.device_handle))
    {
        debug_printf("Bootloader: usb_device_full_init() ---- USB_DeviceClassInit---Failed \n");
        return kStatus_Fail;
    }
    else
    {
      debug_printf("Bootloader: usb_device_full_init() ---- USB_DeviceClassInit---Success\n");
        ...
        debug_printf("Bootloader: usb_device_full_init() ---- usb_device_hid_generic_init before\n");
    ...
    debug_printf("Bootloader: usb_device_full_init() ---- usb_device_hid_generic_init --- After\n");
    /* Install isr, set priority, and enable IRQ. */
    NVIC_SetPriority((IRQn_Type)irqNumber, USB_DEVICE_INTERRUPT_PRIORITY);
    NVIC_EnableIRQ((IRQn_Type)irqNumber);
    
    debug_printf("Bootloader: usb_device_full_init() ---- before USB_DeviceRun \n");
...
#else
        CLOCK_DisableClock(USB_CLOCK_NAME);
#endif
    }
#endif
}
    ...
    *packetLength = 0;
    ...
    do
    {
        if (s_dHidMscActivity[USB_HID_INDEX])
        {
            ...
            reportID = g_device_composite.hid_generic.hid_packet.report.header.reportID;

            // Check the report ID, the first byte of the report buffer.
            if ((reportID != kBootloaderReportID_CommandOut) && (reportID != kBootloaderReportID_DataOut))
            {
                status = kStatus_Fail;
                break;
            }
