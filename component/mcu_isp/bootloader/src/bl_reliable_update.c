...
{
#if BL_TARGET_ROM
    return (g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashTotalSize >> 1);
#elif BL_TARGET_FLASH
#if BL_IS_HARDWARE_SWAP_ENABLED
    return (g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase +
            (g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashTotalSize >> 1) - BL_APP_VECTOR_TABLE_ADDRESS);
#else
    int32_t maxAppSize;
    int32_t maxBackupAppSize;

    maxAppSize = address - get_application_base(kSpecifiedApplicationType_Main);
    maxBackupAppSize = g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase +
                       g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashTotalSize - address;

    maxAppSize = ALIGN_DOWN(maxAppSize, g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashSectorSize);
    maxBackupAppSize = ALIGN_UP(maxBackupAppSize, g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashSectorSize);
...
#if BL_TARGET_ROM
        return g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase +
               (g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashTotalSize >> 1);
#elif BL_TARGET_FLASH
#if BL_IS_HARDWARE_SWAP_ENABLED
        return g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase +
               (g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashTotalSize >> 1) + BL_APP_VECTOR_TABLE_ADDRESS;
...
static bool is_backup_bootloader_valid(void)
{
    ...
    uint32_t bootloaderSizeInByte = get_application_base(kSpecifiedApplicationType_Main) -
                                    g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase;
    uint32_t mainBootloaderBase = g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase;
    uint32_t backupBootloaderBase = g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashBlockBase +
                                    (g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashTotalSize >> 1);
...
static status_t get_swap_indicator_address_if_system_is_in_ready(uint32_t *address)
{
    ...
    status_t result = FLASH_SwapControl(&g_bootloaderContext.allFlashState[kFlashIndex_Main], 0x10,
    ...
    *address = 0;
    if ((result == kStatus_FLASH_Success) && (returnSwapInfo.flashSwapState == kFLASH_SwapStateReady))
    {
        ...
        result = FLASH_ReadResource(&g_bootloaderContext.allFlashState[kFlashIndex_Main],
                                    kFLASH_ResourceRangePflashSwapIfrStart, (uint32_t *)&flashSwapIfrField,
...
#endif
    // Swap flash system
    status = FLASH_Swap(&g_bootloaderContext.allFlashState[kFlashIndex_Main], swapIndicatorAddress,
                        kFLASH_SwapFunctionOptionEnable);
    if ((kStatus_FLASH_SwapIndicatorAddressError == status) || (kStatus_FLASH_AlignmentError == status))
    {
...
status_t software_reliable_update(uint32_t backupApplicationBase)
{
    ...
    applicationSizeInByte =
        ALIGN_UP(applicationSizeInByte, g_bootloaderContext.allFlashState[kFlashIndex_Main].PFlashSectorSize);
