...
class Image():
    ...
    def load(self, path):
        ...
        if self.key_info is not None:
            with open(self.key_info, 'rb') as f:
                self.key_info_block = f.read()

        self.check()

    def add_keyinfo(self):
        if self.key_info_block:
            #self.payload = bytearray(self.payload)
            #self.payload.insert(DEFAULT_KEY_INFO_OFFSET, self.key_info_block)
            self.payload = self.payload[:DEFAULT_KEY_INFO_OFFSET] + self.key_info_block + self.payload[DEFAULT_KEY_INFO_OFFSET + len(self.key_info_block):]
            #self.payload[DEFAULT_KEY_INFO_OFFSET:len(self.key_info_block)] = bytearray(self.key_info_block)  #self.key_info_block
    ...
    def check(self):
        ...
        if (len(self.key_info_block) + IMAGE_HEADER_SIZE) > self.header_size:
            raise Exception("Requested header size is small, can not insert key info block")
        if self.slot_size > 0:
