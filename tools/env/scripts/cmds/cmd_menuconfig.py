...
def mk_sblconfig(filename):
    ...
    sblconfig.write('#ifndef RT_CONFIG_H__\n')
    sblconfig.write('#define RT_CONFIG_H__\n\n')
...
def cmd(args):
    ...
    if not os.path.exists('Kconfig'):
        ...
        print("\n\033[1;31;40m<menuconfig> 命令应当在某一特定 target 目录下执行，例如：\"sbl/target/evkmimxrt1060\"\033[0m")
        print("\033[1;31;40m请确保当前目录为 target 根目录，并且该目录中有 Kconfig 文件。\033[0m\n")

        print ("<menuconfig> command should be used in a bsp root path with a Kconfig file.")
        print ("Example: \"sbl/target/evkmimxrt1060\"")
    ...
    if args.menuconfig_fn:
        ...
        env_kconfig_path = os.path.join(env_root, 'scripts\cmds')
...
def add_parser(sub):
    ...
    parser.add_argument('--generate',
                        help='generate sblconfig.h by .config.',
                        action='store_true',
                        default=False,
                        dest='menuconfig_g')
    ...
    parser.add_argument('--easy',
                        help='easy mode,place kconfig file everywhere,just modify the option env="SBL_ROOT" default "../.."',
                        action='store_true',
                        default=False,
