...
def IARWorkspace(target):
    ...
    # copy template.ewd and rename it as sbl.ewd
    if os.path.exists('iar\\template.ewd'):
        shutil.copy2('iar\\template.ewd', 'iar\\sbl.ewd')
...
def IARVersion():
    ...
    def IARPath():
        ...
        os.environ['SBL_CC'] = 'iar'
        utils.ReloadModule(sblprofile)

        # get iar path
        path = sblprofile.EXEC_PATH
    ...
    if os.path.exists(path):
        ...
        print('Error: get IAR version failed. Please update the IAR installation path in sblprofile.py!')
        exit(-1)
