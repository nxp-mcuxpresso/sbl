...
def walk_kconfig(SBL_ROOT, source_list):
    for parent, dirnames, filenames in os.walk(SBL_ROOT):
        if 'bsp' in parent:
            continue
        if '.git' in parent:
            continue
...
def bsp_copy_files(bsp_root, dist_dir):
    ...
    do_copy_folder(os.path.join(bsp_root), dist_dir,
        ignore_patterns('build', 'dist', '*.pyc', '*.old', '*.map', 'sbl.bin', '.sconsign.dblite', '*.elf', '*.axf', 'cconfig.h'))
...
def bsp_update_sconstruct(dist_dir):
    ...
    with open(os.path.join(dist_dir, 'SConstruct'), 'w') as f:
        for line in data:
            if line.find('SBL_ROOT') != -1:
                if line.find('sys.path') != -1:
                    f.write('# set SBL_ROOT\n')
                    f.write('if not os.getenv("SBL_ROOT"): \n    SBL_ROOT="sbl"\n\n')
            f.write(line)

def bsp_update_kconfig(dist_dir):
    # change SBL_ROOT in Kconfig
...
def bs_update_ide_project(bsp_root, sbl_root, sblide = None):
    import subprocess
    ...
    if sblide == None:
        ...
        item = 'eclipse --project-name=' + sblide['project_name']
        tgt_dict = {item:('gcc', 'gcc')}

    scons_env = os.environ.copy()
    scons_env['SBL_ROOT'] = sbl_root
...
def MkDist_Strip(program, BSP_ROOT, SBL_ROOT, Env):
    global source_list
    ...
    target_list = []
    libcpu_dir = os.path.join(SBL_ROOT, 'libcpu').lower()
    libc_dir = os.path.join(SBL_ROOT, 'components', 'libc', 'compilers').lower()
    sal_dir = os.path.join(SBL_ROOT, 'components', 'net', 'sal_socket').lower()
    ...
    for src in source_list:
        ...
        if src.lower().startswith(SBL_ROOT.lower()):
    ...
    walk_kconfig(SBL_ROOT, source_list)

    # copy all files to target directory
    source_list.sort()
    for src in source_list:
        dst = src.replace(SBL_ROOT, '')
    ...
    print('=> tools')
    do_copy_folder(os.path.join(SBL_ROOT, 'tools'), os.path.join(target_path, 'tools'), ignore_patterns('*.pyc'))
    do_copy_file(os.path.join(SBL_ROOT, 'Kconfig'), os.path.join(target_path, 'Kconfig'))
    do_copy_file(os.path.join(SBL_ROOT, 'AUTHORS'), os.path.join(target_path, 'AUTHORS'))
    do_copy_file(os.path.join(SBL_ROOT, 'COPYING'), os.path.join(target_path, 'COPYING'))
    do_copy_file(os.path.join(SBL_ROOT, 'README.md'), os.path.join(target_path, 'README.md'))
    do_copy_file(os.path.join(SBL_ROOT, 'README_zh.md'), os.path.join(target_path, 'README_zh.md'))

    print('=> %s' % os.path.join('components', 'libc', 'compilers'))
    do_copy_folder(os.path.join(SBL_ROOT, 'components', 'libc', 'compilers'), os.path.join(target_path, 'components', 'libc', 'compilers'))

    if sources_include_sal:
        print('=> %s' % os.path.join('components', 'net', 'sal_socket'))
        do_copy_folder(os.path.join(SBL_ROOT, 'components', 'net', 'sal_socket'), os.path.join(target_path, 'components', 'net', 'sal_socket'))

    # copy all libcpu/ARCH directory
    import sblprofile
    print('=> %s' % (os.path.join('libcpu', sblprofile.ARCH, sblprofile.CPU)))
    do_copy_folder(os.path.join(SBL_ROOT, 'libcpu', sblprofile.ARCH, sblprofile.CPU), os.path.join(target_path, 'libcpu', sblprofile.ARCH, sblprofile.CPU))
    if os.path.exists(os.path.join(SBL_ROOT, 'libcpu', sblprofile.ARCH, 'common')):
        print('=> %s' % (os.path.join('libcpu', sblprofile.ARCH, 'common')))
        do_copy_folder(os.path.join(SBL_ROOT, 'libcpu', sblprofile.ARCH, 'common'), os.path.join(target_path, 'libcpu', sblprofile.ARCH, 'common'))
    do_copy_file(os.path.join(SBL_ROOT, 'libcpu', 'Kconfig'), os.path.join(target_path, 'libcpu', 'Kconfig'))
    do_copy_file(os.path.join(SBL_ROOT, 'libcpu', 'SConscript'), os.path.join(target_path, 'libcpu', 'SConscript'))

    # change SBL_ROOT in SConstruct
...
def MkDist(program, BSP_ROOT, SBL_ROOT, Env, sblide = None):
    ...
    # copy tools directory
    print('=> components')
    do_copy_folder(os.path.join(SBL_ROOT, 'components'), os.path.join(target_path, 'components'))
    ...
    print('=> include')
    do_copy_folder(os.path.join(SBL_ROOT, 'include'), os.path.join(target_path, 'include'))

    # copy all libcpu/ARCH directory
    print('=> libcpu')
    import sblprofile
    do_copy_folder(os.path.join(SBL_ROOT, 'libcpu', sblprofile.ARCH), os.path.join(target_path, 'libcpu', sblprofile.ARCH))
    do_copy_file(os.path.join(SBL_ROOT, 'libcpu', 'Kconfig'), os.path.join(target_path, 'libcpu', 'Kconfig'))
    do_copy_file(os.path.join(SBL_ROOT, 'libcpu', 'SConscript'), os.path.join(target_path, 'libcpu', 'SConscript'))

    # copy src directory
    print('=> src')
    do_copy_folder(os.path.join(SBL_ROOT, 'src'), os.path.join(target_path, 'src'))
    ...
    if sblide == None:
        ...
        bs_update_ide_project(dist_dir, target_path, sblide)
