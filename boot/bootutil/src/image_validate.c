...
#if defined(MCUBOOT_SIGN_ROM)
extern int rom_verify_sig(uint32_t image_start, uint32_t image_size, uint32_t ivt_offset);
#endif
                      ...
                      int seed_len, uint8_t *out_hash)
{
    ...
    while (true) {
        ...
        if (type == IMAGE_TLV_SHA256) {
            ...
            FIH_CALL(rom_verify_sig, fih_rc, fap->fa_off + BOOT_FLASH_BASE,
                                             hdr->ih_img_size, hdr->ih_hdr_size);
            if (fih_not_eq(fih_rc, FIH_SUCCESS)) {
                goto out;
            }
#endif
